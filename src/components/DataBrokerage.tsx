...
### Setting Up a Large Sample Repository (50–500+ hours)

If you're offering a large sample dataset (e.g. 100 hours of egocentric video), follow this workflow:

**1. Organize your data in cloud storage (R2, S3, or GCS):**

\`\`\`
my-dataset-samples/
  videos/
    episode_001.mp4      # 10-20 min clips
    episode_002.mp4
    ...
    episode_400.mp4
  annotations/
    episode_001.json     # language instructions, task labels
    episode_002.json
    ...
  thumbnails/            # optional but recommended for 50+ episodes
    episode_001.jpg
    episode_002.jpg
    ...
  manifest.json          # generated by the script below
\`\`\`

**2. Generate thumbnails (recommended for large datasets):**

With 50+ episodes, thumbnails make the Sample Explorer load fast. Without them, the browser loads video metadata for each card which is slower.

\`\`\`bash
python manifest-generator.py ./my-dataset-samples/ -o manifest.json --thumbnails -v
\`\`\`

**3. Upload everything to your bucket**, including the generated \`manifest.json\` and \`thumbnails/\` folder.

**4. Configure CORS** on your bucket so the explorer can load your videos and thumbnails:
...
Test: \`curl -I -H "Origin: https://humanoids.fyi" https://your-bucket.com/my-dataset-samples/videos/episode_001.mp4\`

**5. Choose 5-10 inline preview clips** that show the diversity of your dataset — different environments, tasks, and lighting conditions. Upload these to Atlas via Import from URL. These are the "storefront window" buyers see in the catalog.

**6. Paste your manifest URL** (e.g. \`https://your-bucket.com/my-dataset-samples/manifest.json\`) in the Sample Repository field and click Validate.

**Annotation format tips:**

Include \`instruction\`, \`tags\`, and \`duration_seconds\` in your annotations for the best explorer experience. The explorer uses these for search, filtering, and display:

\`\`\`json
{
  "instruction": "Pick up the red cup from the counter",
  "task": "pick_and_place",
  "environment": "kitchen",
  "subtasks": [
    { "start_time": 0.0, "end_time": 3.5, "instruction": "Locate cup" },
    { "start_time": 3.5, "end_time": 8.0, "instruction": "Grasp cup" }
  ]
}
\`\`\`

The \`tags\` field in the manifest powers the filter sidebar. Use consistent tag keys across episodes (e.g. always \`environment\` and \`task\`) so buyers can filter effectively.
